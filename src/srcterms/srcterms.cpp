...
void SourceTerms::ISMCooling(const DvceArray5D<Real> &w0, const EOS_Data &eos_data,
  ...
  Real mH = pmy_pack->punit->atomic_mass_unit_cgs;
  Real mu = pmy_pack->punit->mu();
  // assume density unit is nH cm^-3, so no density conversion is needed
  Real e_unit = mu* mH * pmy_pack->punit->velocity_cgs() * pmy_pack->punit->velocity_cgs();
  Real t_unit = pmy_pack->punit->time_cgs();
  Real temp_unit =  e_unit / 1.381e-16; // e/k_b

  par_for("cooling", DevExeSpace(), 0, nmb1, ks, ke, js, je, is, ie,
  KOKKOS_LAMBDA(const int m, const int k, const int j, const int i) {

    // ----------------------------------------------------------
    // Changed from density unit to nH density unit 
    // ----------------------------------------------------------
    ...
    Real nH = w0(m,IDN,k,j,i); // assume nH = density
    Real t_remain = bdt;
    Real e0 = w0(m,IEN,k,j,i);
    Real e_ = w0(m,IEN,k,j,i);
    // sub-cycling cooling
    while (t_remain > 1e-16) {
      Real temp = temp_unit*e_/nH*gm1;
      Real lambda_cooling = nH * nH * ISMCoolFn(temp);
      Real gamma_heating = nH * heating_rate;
      Real dEdt = (lambda_cooling - gamma_heating)*t_unit/e_unit;
      Real dt_cool = fmin(0.05*e_/fabs(dEdt), t_remain);
      e_ -= dEdt*dt_cool;
      t_remain -= dt_cool;
    }

    //Real dE = bdt * (lambda_cooling - gamma_heating)*t_unit/e_unit;
    Real dE = e0 - e_;
    u0(m,IEN,k,j,i) -= dE;
