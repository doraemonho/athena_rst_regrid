...
void MeshBinaryOutput::WriteOutputFile(Mesh *pm, ParameterInput *pin) {
  ...
  int cells = 0;
  if (nout_mbs > 0) {
    int nout1 = (out_params.proj1) ? 1 : outmbs[0].oie - outmbs[0].ois + 1;
    int nout2 = (out_params.proj2) ? 1 : outmbs[0].oje - outmbs[0].ojs + 1;
    int nout3 = (out_params.proj3) ? 1 : outmbs[0].oke - outmbs[0].oks + 1;
  ...
  for (int m=0; m<nout_mbs; ++m) {
    ...
    int &oie = (out_params.proj1) ? ois : outmbs[m].oie;
    int &ojs = outmbs[m].ojs;
    int &oje = (out_params.proj2) ? ojs : outmbs[m].oje;
    int &oks = outmbs[m].oks;
    int &oke = (out_params.proj3) ? oks : outmbs[m].oke;
    ...
    for (int n=0; n<nout_vars; n++) {
      int cnt=0;
      if (out_params.proj1 || out_params.proj2 || out_params.proj3) {
        auto &indcs = pm->mb_indcs;
        auto &size  = pm->pmb_pack->pmb->mb_size;
        if (out_params.proj1) {
          auto dx1 = size.h_view(m).dx1;
          for (int k=oks; k<=oke; k++) {
            for (int j=ojs; j<=oje; j++) {
              tmp_data = 0.0;
              for (int i=indcs.is; i<=indcs.ie; i++) {
                tmp_data += static_cast<float>(outarray(n,m,k-oks,j-ojs,i-ois)) * dx1;
              }
              single_data[cnt] = tmp_data;
              cnt++;
            }
          }
        } else if (out_params.proj2) {
          auto dx2 = size.h_view(m).dx2;
          for (int k=oks; k<=oke; k++) {
            for (int i=ois; i<=oie; i++) {
              tmp_data = 0.0;
              for (int j=indcs.js; j<=indcs.je; j++) {
                tmp_data += static_cast<float>(outarray(n,m,k-oks,j-ojs,i-ois)) * dx2;
              }
              single_data[cnt] = tmp_data;
              cnt++;
            }
          }
        } else if (out_params.proj3) {
          auto dx3 = size.h_view(m).dx3;
          for (int j=ojs; j<=oje; j++) {
            for (int i=ois; i<=oie; i++) {
              tmp_data = 0.0;
              for (int k=indcs.ks; k<=indcs.ke; k++) {
                tmp_data += static_cast<float>(outarray(n,m,k-oks,j-ojs,i-ois)) * dx3;
              }
              single_data[cnt] = tmp_data;
              cnt++;
            }
