...
BaseTypeOutput::BaseTypeOutput(ParameterInput *pin, Mesh *pm, OutputParameters opar) :
  ...
  for (const auto& variable : variables) {
    ...
    // Dust and Synchrtron Stokes parameters - x1 direction
    if (variable.compare("ISM_Pol_x1") == 0) {
      out_params.contains_derived = true;
      out_params.n_derived += 15;
      // emplace all 9 Stokes parameters
      outvars.emplace_back("dust_Qx",0,&(derived_var));
      outvars.emplace_back("dust_Ux",1,&(derived_var));
      outvars.emplace_back("dust_Ix",2,&(derived_var));
      outvars.emplace_back("sync_Qx",3,&(derived_var));
      outvars.emplace_back("sync_Ux",4,&(derived_var));
      outvars.emplace_back("sync_Ix",5,&(derived_var));
      outvars.emplace_back("sync_Qdx",6,&(derived_var));
      outvars.emplace_back("sync_Udx",7,&(derived_var));
      outvars.emplace_back("sync_Idx",8,&(derived_var));

      outvars.emplace_back("dust_Qx_nomask",9,&(derived_var));
      outvars.emplace_back("dust_Ux_nomask",10,&(derived_var));
      outvars.emplace_back("dust_Ix_nomask",11,&(derived_var));
      outvars.emplace_back("sync_Qdx_nomask",12,&(derived_var));
      outvars.emplace_back("sync_Udx_nomask",13,&(derived_var));
      outvars.emplace_back("sync_Idx_nomask",14,&(derived_var));
    }

    // Dust and Synchrtron Stokes parameters - x2 direction
    if (variable.compare("ISM_Pol_x2") == 0) {
      out_params.contains_derived = true;
      out_params.n_derived += 15;
      // emplace all 9 Stokes parameters
      outvars.emplace_back("dust_Qy",0,&(derived_var));
      outvars.emplace_back("dust_Uy",1,&(derived_var));
      outvars.emplace_back("dust_Iy",2,&(derived_var));
      outvars.emplace_back("sync_Qy",3,&(derived_var));
      outvars.emplace_back("sync_Uy",4,&(derived_var));
      outvars.emplace_back("sync_Iy",5,&(derived_var));
      outvars.emplace_back("sync_Qdy",6,&(derived_var));
      outvars.emplace_back("sync_Udy",7,&(derived_var));
      outvars.emplace_back("sync_Idy",8,&(derived_var));

      outvars.emplace_back("dust_Qy_nomask",9,&(derived_var));
      outvars.emplace_back("dust_Uy_nomask",10,&(derived_var));
      outvars.emplace_back("dust_Iy_nomask",11,&(derived_var));
      outvars.emplace_back("sync_Qdy_nomask",12,&(derived_var));
      outvars.emplace_back("sync_Udy_nomask",13,&(derived_var));
      outvars.emplace_back("sync_Idy_nomask",14,&(derived_var));
    }

    // Dust and Synchrtron Stokes parameters - x3 direction
    if (variable.compare("ISM_Pol_x3") == 0) {
      out_params.contains_derived = true;
      out_params.n_derived += 15;
      // emplace all 9 Stokes parameters
      outvars.emplace_back("dust_Qz",0,&(derived_var));
      outvars.emplace_back("dust_Uz",1,&(derived_var));
      outvars.emplace_back("dust_Iz",2,&(derived_var));
      outvars.emplace_back("sync_Qz",3,&(derived_var));
      outvars.emplace_back("sync_Uz",4,&(derived_var));
      outvars.emplace_back("sync_Iz",5,&(derived_var));
      outvars.emplace_back("sync_Qdz",6,&(derived_var));
      outvars.emplace_back("sync_Udz",7,&(derived_var));
      outvars.emplace_back("sync_Idz",8,&(derived_var));

      outvars.emplace_back("dust_Qz_nomask",9,&(derived_var));
      outvars.emplace_back("dust_Uz_nomask",10,&(derived_var));
      outvars.emplace_back("dust_Iz_nomask",11,&(derived_var));
      outvars.emplace_back("sync_Qdz_nomask",12,&(derived_var));
      outvars.emplace_back("sync_Udz_nomask",13,&(derived_var));
      outvars.emplace_back("sync_Idz_nomask",14,&(derived_var));
    }
...
void BaseTypeOutput::LoadOutputData(Mesh *pm) {
  ...
  if (nout_mbs > 0 ) {
    ...
    if (out_params.proj1) {
      nout1 = (indcs.ie - indcs.is + 1);
    }

    if (out_params.proj2) {
      nout2 = (indcs.je - indcs.js + 1);
    }

    if (out_params.proj3) {
      nout3 = (indcs.ke - indcs.ks + 1);
    }
