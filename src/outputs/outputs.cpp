...
Outputs::Outputs(ParameterInput *pin, Mesh *pm) {
  ...
  for (auto it = pin->block.begin(); it != pin->block.end(); ++it) {
    if (it->block_name.compare(0, 6, "output") == 0) {
      ...
      // read the project options
      opar.proj1 = pin->GetOrAddBoolean(opar.block_name, "proj1", false);
      opar.proj2 = pin->GetOrAddBoolean(opar.block_name, "proj2", false);
      opar.proj3 = pin->GetOrAddBoolean(opar.block_name, "proj3", false);
      if (opar.proj1) {
        opar.d_mask_x1 = pin->GetOrAddReal(opar.block_name, "d_mask", 40.0);
      }
      if (opar.proj2) {
        opar.d_mask_x2 = pin->GetOrAddReal(opar.block_name, "d_mask", 40.0);
      }
      if (opar.proj3) {
        opar.d_mask_x3 = pin->GetOrAddReal(opar.block_name, "d_mask", 40.0);
      }
