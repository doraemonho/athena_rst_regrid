...
void BaseTypeOutput::ComputeDerivedVariable(std::string name, Mesh *pm) {
  ...
  //  Dust & Synchrotron Polarization Sotokes Parameters
  // along x, y, z directions
  if (name.compare("ISM_Pol_x1") == 0) {
    // 0 = dust_Q_{x}, 9 = dust_Q_{x} (no masking)
    // 1 = dust_U_{x}, 10 = dust_U_{x} (no masking)
    // 2 = dust_I_{x}, 11 = dust_I_{x} (no masking)
    // 3 = sync_Q_{x}
    // 4 = sync_U_{x}
    // 5 = snyc_I_{x}
    // 6 = snyc_Qd_{x}, 12 = snyc_Qd_{x} (no masking)
    // 7 = sync_Ud_{x}, 13 = sync_Ud_{x} (no masking)
    // 8 = snyc_Id_{x}, 14 = snyc_Id_{x} (no masking)
    Kokkos::realloc(derived_var, nmb, 18, n3, n2, n1);
    auto dv = derived_var;
    auto &bcc = pm->pmb_pack->pmhd->bcc0;
    auto &b = pm->pmb_pack->pmhd->b0;
    auto &w0_ = pm->pmb_pack->pmhd->w0;
    Real d_mask_x1 = out_params.d_mask_x1;
    Real B_in_mu_G = sqrt(pm->pmb_pack->punit->pressure_cgs()*4*M_PI);
    Real n_in_nH = pm->pmb_pack->punit->density_cgs()/pm->pmb_pack->punit->mu()
                  /pm->pmb_pack->punit->atomic_mass_unit_cgs;

    par_for("ISM_Pol_x1", DevExeSpace(), 0, (nmb-1), ks, ke, js, je, is, ie,
    KOKKOS_LAMBDA(int m, int k, int j, int i) {
      Real  d = w0_(m,IDN,k,j,i);
      Real Bx = bcc(m,IBX,k,j,i);
      Real By = bcc(m,IBY,k,j,i);
      Real Bz = bcc(m,IBZ,k,j,i);
      Real BB = SQR(Bx) + SQR(By) + SQR(Bz);
      Real d_nomask = d;
      if (d*n_in_nH > d_mask_x1){
        d  = 0.0;
      }

      // Dust Stokes Parameters
      // Assuming emissivity e = rho
      dv(m,0,k,j,i) = d*(Bz*Bz - By*By)/BB;  // Q_{d,x} \propto e * (Bz^2 - By^2) / B^2
      dv(m,1,k,j,i) = -2.0*d*By*Bz/BB;       // U_{d,x} \propto -2 * e * By * Bz / B^2
      dv(m,2,k,j,i) = d;                     // I_{d,x} = \int rho dx

      // Simplified Synchrotron Stokes Parameters
      // Assuming emissivity e = b^n, n \approx 0.02667s^2 + 0.166s + 1.44,\: s = log_{10}( b [\mu G] )
      // b = b_\perp
      Real bb = SQR(Bz) + SQR(By);
      Real b = sqrt(SQR(Bz) + SQR(By));
      Real s = log10(b*B_in_mu_G);
      Real n = 0.02667*s*s + 0.166*s + 1.44;
      Real e = pow(b,n);
      Real c = 0.81; // constant for the emissivity of  synchrotron radiation
      // primary synchrotron polarization
      dv(m,3,k,j,i) = e*(Bz*Bz - By*By)/bb;  // Q_{s,x} \propto e * (Bz^2 - By^2) / B^2
      dv(m,4,k,j,i) = -2.0*e*By*Bz/bb;       // U_{s,x} \propto -2 * e * By * Bz / B^2
      dv(m,5,k,j,i) = c*e;                   // I_{s,x} = \int c*b^n dx
      // secondary synchrotron polarization
      dv(m,6,k,j,i) = e*d*(Bz*Bz - By*By)/bb;  // Q_{sd,x} \propto e * (Bz^2 - By^2) / B^2
      dv(m,7,k,j,i) =  -2.0*e*d*By*Bz/bb;      // U_{sd,x} \propto -2 * e * By * Bz / B^2
      dv(m,8,k,j,i) = c*e*d;                   // I_{sd,x} = \int c*b^n*rho dx

      // no masking version
      dv(m,9,k,j,i) = d_nomask*(Bz*Bz - By*By)/BB;     // Q_{d,x} \propto e * (Bz^2 - By^2) / B^2
      dv(m,10,k,j,i) = -2.0*d_nomask*By*Bz/BB;         // U_{d,x} \propto -2 * e * By * Bz / B^2
      dv(m,11,k,j,i) = d_nomask;                       // I_{d,x} = \int rho dx

      dv(m,12,k,j,i) = e*d_nomask*(Bz*Bz - By*By)/bb;  // Q_{sd,x} \propto e * (Bz^2 - By^2) / B^2
      dv(m,13,k,j,i) =  -2.0*e*d_nomask*By*Bz/bb;      // U_{sd,x} \propto -2 * e * By * Bz / B^2
      dv(m,14,k,j,i) = c*e*d_nomask;                   // I_{sd,x} = \int c*b^n*rho dx
    });
  }

  if (name.compare("ISM_Pol_x2") == 0) {
    // 0 = dust_Q_{y}, 9 = dust_Q_{y} (no masking)
    // 1 = dust_U_{y}, 10 = dust_U_{y} (no masking)
    // 2 = dust_I_{y}, 11 = dust_I_{y} (no masking)
    // 3 = sync_Q_{y}
    // 4 = sync_U_{y}
    // 5 = snyc_I_{y}
    // 6 = snyc_Qd_{y}, 12 = snyc_Qd_{y} (no masking)
    // 7 = sync_Ud_{y}, 13 = sync_Ud_{y} (no masking)
    // 8 = snyc_Id_{y}, 14 = snyc_Id_{y} (no masking)
    Kokkos::realloc(derived_var, nmb, 18, n3, n2, n1);
    auto dv = derived_var;
    auto &bcc = pm->pmb_pack->pmhd->bcc0;
    auto &b = pm->pmb_pack->pmhd->b0;
    auto &w0_ = pm->pmb_pack->pmhd->w0;
    Real d_mask_x2 = out_params.d_mask_x2;
    Real B_in_mu_G = sqrt(pm->pmb_pack->punit->pressure_cgs()*4*M_PI);
    Real n_in_nH = pm->pmb_pack->punit->density_cgs()/pm->pmb_pack->punit->mu()
                  /pm->pmb_pack->punit->atomic_mass_unit_cgs;

    par_for("ISM_Pol_x2", DevExeSpace(), 0, (nmb-1), ks, ke, js, je, is, ie,
    KOKKOS_LAMBDA(int m, int k, int j, int i) {
      Real  d = w0_(m,IDN,k,j,i);
      Real Bx = bcc(m,IBX,k,j,i);
      Real By = bcc(m,IBY,k,j,i);
      ...
      Real BB = SQR(Bx) + SQR(By) + SQR(Bz);
      Real d_nomask = d;
      if (d*n_in_nH > d_mask_x2){
        d  = 0.0;
      }
      // Dust Stokes Parameters
      // Assuming emissivity e = rho
      dv(m,0,k,j,i) = d*(Bx*Bx - Bz*Bz)/BB;  // Q_{d,y} \propto e * (Bx^2 - Bz^2) / B^2
      dv(m,1,k,j,i) = -2.0*d*Bz*Bx/BB;       // U_{d,y} \propto -2 * e * Bz * Bx / B^2
      dv(m,2,k,j,i) = d;                      // I_{d,y} = \int rho dy

      // Simplified Synchrotron Stokes Parameters
      // Assuming emissivity e = b^n, n \approx 0.02667s^2 + 0.166s + 1.44,\: s = log_{10}( b [\mu G] )
      // b = b_\perp
      Real bb = SQR(Bx) + SQR(Bz);
      Real b = sqrt(SQR(Bx) + SQR(Bz));
      Real s = log10(b*B_in_mu_G);
      Real n = 0.02667*s*s + 0.166*s + 1.44;
      Real e = pow(b,n);
      Real c = 0.81; // constant for the emissivity of  synchrotron radiation
      ...
      // primary synchrotron polarization
      dv(m,3,k,j,i) =  e*(Bx*Bx - Bz*Bz)/bb;   // Q_{s,y} \propto e * (Bx^2 - Bz^2) / B^2
      dv(m,4,k,j,i) = -2.0*e*Bx*Bz/bb;         // U_{s,y} \propto -2 * e * Bz * Bx / B^2
      dv(m,5,k,j,i) = c*e;                     // I_{s,y} = \int c*b^n dy
      // secondary synchrotron polarization
      dv(m,6,k,j,i) = e*d*(Bx*Bx - Bz*Bz)/bb;  // Q_{sd,y} \propto e * (Bx^2 - Bz^2) / B^2
      dv(m,7,k,j,i) =  -2.0*e*d*Bx*Bz/bb;      // U_{sd,y} \propto -2 * e * Bz * Bx / B^2
      dv(m,8,k,j,i) = c*e*d;                   // I_{sd,y} = \int c*b^n*rho dy

      // no masking version
      dv(m,9,k,j,i) = d_nomask*(Bx*Bx - Bz*Bz)/BB;     // Q_{d,y} \propto e * (Bx^2 - Bz^2) / B^2
      dv(m,10,k,j,i) = -2.0*d_nomask*Bx*Bz/BB;         // U_{d,y} \propto -2 * e * Bz * Bx / B^2
      dv(m,11,k,j,i) = d_nomask;                       // I_{d,y} = \int rho dy
      dv(m,12,k,j,i) = e*d_nomask*(Bx*Bx - Bz*Bz)/bb;  // Q_{sd,y} \propto e * (Bx^2 - Bz^2) / B^2
      dv(m,13,k,j,i) =  -2.0*e*d_nomask*Bx*Bz/bb;      // U_{sd,y} \propto -2 * e * Bz * Bx / B^2
      dv(m,14,k,j,i) = c*e*d_nomask;                   // I_{sd,y} = \int c*b^n*rho dy
    });
  }

  if (name.compare("ISM_Pol_x3") == 0) {
    // 0 = dust_Q_{z}, 9 = dust_Q_{z} (no masking)
    // 1 = dust_U_{z}, 10 = dust_U_{z} (no masking)
    // 2 = dust_I_{z}, 11 = dust_I_{z} (no masking)
    // 3 = sync_Q_{z}
    // 4 = sync_U_{z}
    // 5 = snyc_I_{z}
    // 6 = snyc_Qd_{z}, 12 = snyc_Qd_{z} (no masking)
    // 7 = sync_Ud_{z}, 13 = sync_Ud_{z} (no masking)
    // 8 = snyc_Id_{z}, 14 = snyc_Id_{z} (no masking)
    Kokkos::realloc(derived_var, nmb, 18, n3, n2, n1);
    auto dv = derived_var;
    auto &bcc = pm->pmb_pack->pmhd->bcc0;
    auto &b = pm->pmb_pack->pmhd->b0;
    auto &w0_ = pm->pmb_pack->pmhd->w0;
    Real d_mask_x3 = out_params.d_mask_x3;
    Real B_in_mu_G = sqrt(pm->pmb_pack->punit->pressure_cgs()*4*M_PI);
    Real n_in_nH = pm->pmb_pack->punit->density_cgs()/pm->pmb_pack->punit->mu()
                  /pm->pmb_pack->punit->atomic_mass_unit_cgs;

    par_for("ISM_Pol_x3", DevExeSpace(), 0, (nmb-1), ks, ke, js, je, is, ie,
    KOKKOS_LAMBDA(int m, int k, int j, int i) {
      Real  d = w0_(m,IDN,k,j,i);
      Real Bx = bcc(m,IBX,k,j,i);
      Real By = bcc(m,IBY,k,j,i);
      ...
      Real BB = SQR(Bx) + SQR(By) + SQR(Bz);
      Real d_nomask = d;
      if (d*n_in_nH > d_mask_x3){
        d  = 0.0;
      }
      // Dust Stokes Parameters
      // Assuming emissivity e = rho
      dv(m,0,k,j,i) = d*(By*By - Bx*Bx)/BB;  // Q_{d,z} \propto e * (By^2 - Bx^2) / B^2
      dv(m,1,k,j,i) = -2.0*d*Bx*By/BB;       // U_{d,z} \propto -2 * e * Bx * By / B^2
      dv(m,2,k,j,i) = d;                     // I_{d,z} = \int rho dz

      // Simplified Synchrotron Stokes Parameters
      // Assuming emissivity e = b^n, n \approx 0.02667s^2 + 0.166s + 1.44,\: s = log_{10}( b [\mu G] )
      // b = b_\perp
      Real bb = SQR(Bx) + SQR(By);
      Real b = sqrt(SQR(Bx) + SQR(By));
      Real s = log10(b*B_in_mu_G);
      Real n = 0.02667*s*s + 0.166*s + 1.44;
      Real e = pow(b,n);
      Real c = 0.81; // constant for the emissivity of  synchrotron radiation
      ...
      // primary synchrotron polarization
      dv(m,3,k,j,i) =  e*(By*By - Bx*Bx)/bb;  // Q_{s,z} \propto e * (By^2 - Bx^2) / B^2
      dv(m,4,k,j,i) = -2.0*e*Bx*By/bb;        // U_{s,z} \propto -2 * e * Bx * By / B^2
      dv(m,5,k,j,i) = c*e;                    // I_{s,z} = \int c*b^n dz
      
      // secondary synchrotron polarization
      dv(m,6,k,j,i) = e*d*(By*By - Bx*Bx)/bb; // Q_{sd,z} \propto e * d * (By^2 - Bx^2) / B^2
      dv(m,7,k,j,i) =  -2.0*e*d*Bx*By/bb;     // U_{sd,z} \propto -2 * e * d * Bx * By / B^2
      dv(m,8,k,j,i) = c*e*d;                  // I_{sd,z} = \int c*b^n*rho dz

      // no masking version
      dv(m,9,k,j,i) = d_nomask*(By*By - Bx*Bx)/BB;    // Q_{d,z} \propto e * (By^2 - Bx^2) / B^2
      dv(m,10,k,j,i) = -2.0*d_nomask*Bx*By/BB;        // U_{d,z} \propto -2 * e * Bx * By / B^2
      dv(m,11,k,j,i) = d_nomask;                      // I_{d,z} = \int rho dz
      dv(m,12,k,j,i) = e*d_nomask*(By*By - Bx*Bx)/bb; // Q_{sd,z} \propto e * d * (By^2 - Bx^2) / B^2
      dv(m,13,k,j,i) =  -2.0*e*d_nomask*Bx*By/bb;     // U_{sd,z} \propto -2 * e * d * Bx * By / B^2
      dv(m,14,k,j,i) = c*e*d_nomask;                  // I_{sd,z} = \int c*b^n*rho dz
    });
  }
